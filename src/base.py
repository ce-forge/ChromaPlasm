...
class Base:
    ...
    def recalculate_geometry(self, final_calculation=True):
        """Calculates the definitive geometry for the base, including smart armor."""
        ...
        self.current_core_pixels = list(core_set)
        if not core_set: 
            self.current_armor_pixels, self.all_base_pixels = [], set()
            return
            ...
            for cy, cx in core_local:
                for dy in range(-self.armor_thickness, self.armor_thickness + 1):
                    for dx in range(-self.armor_thickness, self.armor_thickness + 1):
                        cand = (cy + dy, cx + dx)
                        if cand not in core_local and cand in ext_local:
                            armor_local.add(cand)
            armor_set = set((y + min_y, x + min_x) for y, x in armor_local)
        else: # Fast preview armor
            for cy, cx in self.current_core_pixels:
                for dy in range(-self.armor_thickness, self.armor_thickness + 1):
                    for dx in range(-self.armor_thickness, self.armor_thickness + 1):
                        cand = (cy + dy, cx + dx)
                        if cand not in core_set: armor_set.add(cand)
