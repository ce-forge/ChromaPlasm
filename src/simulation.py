...
# This is the new, high-performance Numba function. It is the core of the simulation.
@jit(nopython=True, parallel=True, fastmath=True, cache=True)
def _numba_simulation_step(agent_count, agent_positions, agent_headings, agent_teams, agent_health,
                             vfx_events, logic_grid, object_grid,
                             all_pheromone_grids, # A 3D array of all team pheromones
                             alliance_map,        # The map of team alliances
                             grid_h, grid_w,
                             r_sens_angle, r_rot_angle, r_sens_dist, # These will be made team-specific later
    ...
    for i in range(agent_count):
        if agent_health[i] > 0:
            grid_x, grid_y = int(agent_positions[i, 1] / SPATIAL_GRID_CELL_SIZE), int(agent_positions[i, 0] / SPATIAL_GRID_CELL_SIZE)
    ...
    for i in prange(agent_count):
                ...
                for k in range(-1, 2):
                    ...
                    if 0 <= check_grid_y < spatial_grid_h and 0 <= check_grid_x < spatial_grid_w:
                        ...
                        while current_agent_idx != -1:
                            ...
                            if alliance_map[other_team_id] != agent_alliance_id: # Check alliance
                ...
                for sx in range(int(x) - 50, int(x) + 50):
                    if 0 <= sy < grid_h and 0 <= sx < grid_w:
                        ...
                        if BASE_ARMOR_OFFSET <= terrain_id < BASE_CORE_OFFSET:
                            ...
                            if alliance_map[base_team_id] != agent_alliance_id: # Check alliance
        ...
        if target_object_idx != -1 and alliance_map[agent_teams[target_object_idx]] != agent_alliance_id:
            ...
            for _ in range(5):
                ...
                check_y, check_x = int(y + np.sin(rand_heading)), int(x + np.cos(rand_heading))
...
class Simulation:
    def __init__(self, config, vfx_manager, audio_manager):
        ...
        for team in TEAMS:
            self.pheromone_managers[team['id']].color = team['pheromone_color']
        self.pheromone_surfaces = { team['id']: pygame.Surface((self.grid_size[1], self.grid_size[0]), flags=pygame.SRCALPHA) for team in TEAMS }
        
        self.alliance_map = np.arange(len(TEAMS)) # Default to Free For All

        self.team_params_overrides = {}
        ...
        self.agent_teams = np.zeros(self.max_agents, dtype=np.int8) # Now stores team_id
    ...
    def _compile_team_params(self):
        self.params_red = self.get_params_for_team(1) # Crimson
        self.params_blue = self.get_params_for_team(0) # Azure

    def get_params_for_team(self, team_id):
        params = {
            'sensor_angle_degrees': self.get_param(team_id, 'sensor_angle_degrees'),
            'rotation_angle_degrees': self.get_param(team_id, 'rotation_angle_degrees'),
            'sensor_distance': self.get_param(team_id, 'sensor_distance'),
            'pheromone_deposit_amount': self.get_param(team_id, 'pheromone_deposit_amount'),
            'sensor_angle_rad': np.deg2rad(self.get_param(team_id, 'sensor_angle_degrees')),
            'rotation_angle_rad': np.deg2rad(self.get_param(team_id, 'rotation_angle_degrees')),
        }
        return params
    ...
    def _initialize_bases(self):
        ...
        try:
            ...
            for base_config in layout_data.get('initial_layout', []):
                new_base = Base(
                    team_name=base_config['team'],
                    pivot_y=base_config['pivot'][0], pivot_x=base_config['pivot'][1],
                    shape_name=base_config['shape_name'], config=self.config,
    ...
    def draw_bases_to_grid(self):
        ...
        for base in self.bases:
            ...
            for y, x in base.current_armor_pixels:
                 if 0 <= y < SIM_HEIGHT and 0 <= x < SIM_WIDTH and (y, x) not in all_core_pixels:
    ...
    def add_soldier(self, y, x, team_id, heading):
        if self.agent_count < self.max_agents:
            ...
            self.agent_teams[self.agent_count] = team_id
    ...
    def get_team_base_health(self, team_name):
        ...
        return np.sum(self.render_grid == (BASE_ARMOR_OFFSET + team_id))
    ...
    def step(self, frame_count):
        ...
        r_params, b_params = self.params_red, self.params_blue # Kept for agent behavior compatibility for now
        self.agent_positions, self.agent_headings, self.agent_health, self.vfx_events, post_combat_grid = _numba_simulation_step(
            self.agent_count, self.agent_positions, self.agent_headings, self.agent_teams, self.agent_health, 
            self.vfx_events, logic_grid, self.object_grid, all_phero_grids, self.alliance_map,
            ...
            self.config.combat_chance, self.frame_count,
            self.config.enemy_sense_radius**2, self.config.base_attack_radius**2, self.config.ai_update_interval
        )
        ...
        # --- PERFORMANCE FIX: Only loop over teams that are actually in the simulation ---
        alive_mask = self.agent_health[:self.agent_count] > 0
        active_team_ids = np.unique(self.agent_teams[:self.agent_count][alive_mask])

        for team_id in active_team_ids:
            manager = self.pheromone_managers[team_id]
            team_agent_mask = alive_mask & (self.agent_teams[:self.agent_count] == team_id)
            if np.any(team_agent_mask):
                deposit_amount = self.get_param(team_id, 'pheromone_deposit_amount')
                manager.deposit(self.agent_positions[:self.agent_count][team_agent_mask], deposit_amount)
        
        # We still need to update all managers for decay, but this is much cheaper
        for manager in self.pheromone_managers.values():
            manager.update(self.frame_count)
        
        if self.frame_count % 2 == 0:
            for team_id in active_team_ids:
                self.pheromone_surfaces[team_id] = self.pheromone_managers[team_id].get_render_surface()
        ...
        for i in range(self.agent_count):
            ...
            if 0 <= y < self.grid_size[0] and 0 <= x < self.grid_size[1]:
                if self.render_grid[y, x] == EMPTY:
                    self.render_grid[y, x] = SOLDIER_OFFSET + self.agent_teams[i]
