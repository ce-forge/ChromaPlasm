...
def _numba_simulation_step(agent_count, agent_positions, agent_headings, agent_teams, agent_health,
    ...
    for i in prange(agent_count):
        ...
        if target_object_idx != -1 and alliance_map[agent_teams[target_object_idx]] != agent_alliance_id:
            ...
            base_damage_events[i, 0] = 1
            base_damage_events[i, 1] = base_team_id
...
class Simulation:
    def __init__(self, config, vfx_manager, audio_manager):
        ...
        self.alliance_map = np.arange(len(TEAMS))
        ...
        self.base_damage_events = np.zeros((self.max_agents, 2), dtype=np.int32)
        self.agent_count = 0
    ...
    def step(self, frame_count):
        ...
        for i in range(self.agent_count):
            ...
            if self.base_damage_events[i, 0] == 1:
                damaged_team_id = self.base_damage_events[i, 1]
                for base in self.bases:
        ...
        for team_id in active_team_ids:
            ...
            if np.any(team_agent_mask):
                manager.deposit(self.agent_positions[:self.agent_count][team_agent_mask], self.get_param(team_id, 'pheromone_deposit_amount'))
        
        for manager in self.pheromone_managers.values(): manager.update(self.frame_count)
        if self.frame_count % 2 == 0:
            for team_id in active_team_ids: self.pheromone_surfaces[team_id] = self.pheromone_managers[team_id].get_render_surface()
