...
class LiveRenderer:
    ...
    def draw(self, screen, sim, vfx_manager, viewport, show_pheromones, 
        ...
        if active_teams:
            groups = {}
            [groups.setdefault(sim.alliance_map[tid], []).append(tid) for tid in active_teams]
            ...
            for aid in sorted(groups.keys()):
                ...
                for team_id in groups[aid]:
                    ...
                    try:
                        health = sim.get_team_base_health(team['name'])
                        h_font_size = int(symbol_size * 0.65)
                        health_font = pygame.font.Font(self.font_path, h_font_size)
                        health_font.set_bold(True)
                        health_surf = health_font.render(str(health), True, (0,0,0))
                        health_rect = health_surf.get_rect(center=symbol_rect.center)
        ...
        if show_pheromones:
            [world_surface.blit(p_surf, (0, 0), special_flags=pygame.BLEND_RGBA_ADD) for p_surf in sim.pheromone_surfaces.values()]
            
        for p in vfx_manager.particles:
            if hasattr(p, 'y') and p.y is not None:
                alpha = int(255 * (p.lifespan / p.max_lifespan))
                size = max(1.0, p.radius * PIXEL_SCALE * 0.5)
        ...
        blit_x = (viewport.rect.width / 2) - viewport.offset_x * PIXEL_SCALE * zoom
        blit_y = (viewport.rect.height / 2) - viewport.offset_y * PIXEL_SCALE * zoom
        
        viewport_surface.blit(scaled_final_surf, (blit_x, blit_y))
        pygame.draw.rect(viewport_surface, (200, 200, 220), pygame.Rect(blit_x, blit_y, scaled_final_surf.get_width(), scaled_final_surf.get_height()), 2)
        
        if selected_object or is_editing_spawns:
            game_area_screen_x = blit_x
            game_area_screen_y = blit_y + (VIDEO_TOP_MARGIN * zoom)
            
            if selected_object:
                highlight_color = (255, 255, 0)
                pixel_size = max(1, PIXEL_SCALE * zoom)
                for y, x in selected_object.rim_pixels:
                    pygame.draw.rect(viewport_surface, highlight_color, (game_area_screen_x + (x*PIXEL_SCALE*zoom), game_area_screen_y + (y*PIXEL_SCALE*zoom), pixel_size, pixel_size), 1)

            if is_editing_spawns:
                 port_color = (255, 255, 0)
                 for y, x in selected_object.exit_ports:
                    pygame.draw.circle(viewport_surface, port_color, (game_area_screen_x + (x*PIXEL_SCALE*zoom), game_area_screen_y + (y*PIXEL_SCALE*zoom)), int(max(2, 6 * zoom)), 2)
    ...
    def _create_background_surface(self):
        background = pygame.Surface((SIM_WIDTH, SIM_HEIGHT))
        background.fill((16, 16, 26))
        center_x, center_y = SIM_WIDTH // 2, SIM_HEIGHT // 2
        
        grid_color_1 = (35, 35, 50)
        grid_spacing_1 = 10
        ...
        for i in range(num_x_lines_1):
            pygame.draw.line(background, grid_color_1, (offset_x_1 + i * grid_spacing_1, 0), (offset_x_1 + i * grid_spacing_1, SIM_HEIGHT))
        for i in range(num_y_lines_1):
            pygame.draw.line(background, grid_color_1, (0, offset_y_1 + i * grid_spacing_1), (SIM_WIDTH, offset_y_1 + i * grid_spacing_1))
            
        grid_color_2 = (45, 55, 80)
        line_thickness_2 = 2
        grid_spacing_2 = 100
        ...
        for i in range(num_x_lines_2):
            pygame.draw.line(background, grid_color_2, (offset_x_2 + i * grid_spacing_2, 0), (offset_x_2 + i * grid_spacing_2, SIM_HEIGHT), line_thickness_2)
        for i in range(num_y_lines_2):
            y_pos = offset_y_2 + i * grid_spacing_2
            pygame.draw.line(background, grid_color_2, (0, y_pos), (SIM_WIDTH, y_pos), line_thickness_2)
        ...
        vignette_strength = 10
        for r in range(0, int(max_dist), 3):
            alpha = int(vignette_strength * (r / max_dist)**2)
            color = (0, 0, 0, alpha)
