...
class Dashboard:
    def __init__(self):
        ...
        self.frame_count = 0; self.show_pheromones = True
        self.current_mode = 'SIMULATION'; self.is_editing_spawns = False; self.editor_port_mode = 'DRAG'
        self.dragged_object, self.drag_type, self.selected_port_index = None, None, -1
        self.drag_offset = (0, 0); self.shorts_title_text = self.presentation_params.question_text
        self.is_paused = False; self.sim_speed = 1; self.speed_options = [1, 2, 4, 8]
        
        pygame.init()
        self.screen = pygame.display.set_mode((DASHBOARD_WIDTH, DASHBOARD_HEIGHT), pygame.RESIZABLE)
        pygame.display.set_caption("ChromaPlasm Dashboard"); self.clock = pygame.time.Clock(); self.is_running = True

        self.ui_manager = pygame_gui.UIManager((DASHBOARD_WIDTH, DASHBOARD_HEIGHT), 'theme.json')
        self.viewport = Viewport(pygame.Rect(0, 0, 1, 1)); self.selected_object = None
    ...
    def _refresh_dynamic_panels(self):
        ...
        self.ui_elements['alliance'] = {}; self.ui_elements['global'] = {}; self.ui_elements['presentation'] = {}
        y_offset = 10
        pygame_gui.elements.UILabel(relative_rect=pygame.Rect(10, y_offset, content_width - 20, 30), text="Alliance Settings", manager=self.ui_manager, container=self.sim_ui_group); y_offset += 35
        ...
        self.ui_elements['alliance']['2_teams_button'] = pygame_gui.elements.UIButton(relative_rect=pygame.Rect(200, y_offset, 180, 30), text="Two Teams", manager=self.ui_manager, container=self.sim_ui_group); y_offset += 40
        ...
        for team_id in active_team_ids:
            team = TEAMS[team_id]; hex_color = '#%02x%02x%02x' % team['color']
            ...
            self.ui_elements['alliance'][f'team_{team_id}_dropdown'] = dropdown; y_offset += 30
        
        y_offset += 10; pygame_gui.elements.UIPanel(relative_rect=pygame.Rect(10, y_offset, content_width - 20, 2), manager=self.ui_manager, container=self.sim_ui_group); y_offset += 10
        pygame_gui.elements.UILabel(relative_rect=pygame.Rect(10, y_offset, content_width-20, 30), text="Global Simulation Parameters", manager=self.ui_manager, container=self.sim_ui_group); y_offset += 35
        ...
        for name, p_range in sim_params.items():
            ...
            self.ui_elements['global'][name] = {'slider': slider, 'label': label, 'is_int': is_int}; y_offset += 35
        
        y_offset += 10; pygame_gui.elements.UIPanel(relative_rect=pygame.Rect(10, y_offset, content_width - 20, 2), manager=self.ui_manager, container=self.sim_ui_group); y_offset += 10
        pygame_gui.elements.UILabel(relative_rect=pygame.Rect(10, y_offset, content_width-20, 30), text="Presentation Tuning", manager=self.ui_manager, container=self.sim_ui_group); y_offset += 35
        ...
        for name, p_range in presentation_sliders.items():
            ...
            self.ui_elements['presentation'][name] = {'slider': slider, 'label': label, 'is_int': is_int}; y_offset += 35
    ...
    def _update_alliance_dropdowns(self):
        ...
        for team_id, alliance_id in enumerate(self.alliance_map):
            ...
            if dropdown: dropdown.rebuild(alliance_group_options, f"Group {alliance_id + 1}")
    ...
    def toggle_editor_mode(self, initial=False):
        ...
        if self.current_mode == 'EDITOR':
            self.is_paused = True; self.play_pause_button.disable(); self.toggle_editor_button.set_text('Exit Editor')
            self.sim_ui_group.hide(); self.editor_ui_group.show()
            self.selection_panel.set_relative_position((10, 290))
        else:
            self.play_pause_button.enable(); self.toggle_editor_button.set_text('Enter Editor')
            self.sim_ui_group.show(); self.editor_ui_group.hide()
            self._refresh_dynamic_panels()
            self.is_editing_spawns = False; self.editor_port_mode = 'DRAG'
    ...
    def update_selection_panel(self):
        ...
        if not self.selected_object: self.selection_panel.hide(); return
        self.selection_panel.show(); base = self.selected_object
        container = self.selection_panel; content_width = container.get_container().get_rect().width; y_offset = 5
        pygame_gui.elements.UILabel(relative_rect=pygame.Rect(10, y_offset, content_width - 20, 30), text="Selected Base Properties", manager=self.ui_manager, container=container); y_offset += 35
        def create_entry(name, value, y_pos):
            ...
            entry.set_text(str(value)); self.ui_elements['selection'][name] = entry
            return entry
        if self.current_mode == 'EDITOR':
            ...
            self.ui_elements['selection']['team_dropdown'] = pygame_gui.elements.UIDropDownMenu(options_list=team_names, starting_option=base.team_name, relative_rect=pygame.Rect(180, y_offset, content_width - 190, 30), manager=self.ui_manager, container=container); y_offset += 40
            ...
            self.ui_elements['selection']['shape_dropdown'] = pygame_gui.elements.UIDropDownMenu(options_list=['Y', 'N', 'BOX', 'ARROWHEAD'], starting_option=base.shape_name, relative_rect=pygame.Rect(180, y_offset, content_width - 190, 30), manager=self.ui_manager, container=container); y_offset += 40
            ...
            size_map = {0: 4.0, 1: 6.0, 2: 8.0, 3: 10.0}; text_map = {0: "50%", 1: "75%", 2: "100%", 3: "125%"}
            val_to_scale = {v: k for k, v in size_map.items()}; current_val = val_to_scale.get(base.scale, 2)
            ...
            self.ui_elements['selection']['size'] = {'slider': slider, 'label': label, 'map': size_map, 'text_map': text_map}; y_offset += 40
            ...
            self.ui_elements['selection']['modify_spawns_button'] = pygame_gui.elements.UIButton(relative_rect=pygame.Rect(10, y_offset, content_width - 20, 40), text=btn_text, manager=self.ui_manager, container=container); y_offset += 45
            if self.is_editing_spawns:
                ...
                self.ui_elements['selection']['add_port_button'] = add_btn; self.ui_elements['selection']['delete_port_button'] = del_btn; y_offset += 35
            ...
            create_entry('spawn_rate', self.simulation.get_param(base.team_id, 'spawn_rate'), y_offset); y_offset += 40
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame_gui.UI_BUTTON_PRESSED:
                if event.ui_element == self.play_pause_button: self.toggle_pause()
                    ...
                    new_base = self.simulation.add_new_base(); self.selected_object = new_base; self.is_editing_spawns = False; self.editor_port_mode = 'DRAG'; self.update_selection_panel()
                elif event.ui_element == self.editor_delete_button: self.simulation.delete_base(self.selected_object); self.selected_object = None; self.update_selection_panel()
                elif event.ui_element == self.ui_elements['alliance'].get('ffa_button'):
                    self.alliance_map = list(range(len(TEAMS))); self._refresh_dynamic_panels(); self._update_simulation_alliances()
                elif event.ui_element == self.ui_elements['alliance'].get('2_teams_button'):
                    self.alliance_map = [i % 2 for i in range(len(TEAMS))]; self._refresh_dynamic_panels(); self._update_simulation_alliances()                
                if self.selected_object:
                    ...
                    if self.is_editing_spawns:
                        if event.ui_element == self.ui_elements['selection'].get('add_port_button'): self.editor_port_mode = 'DRAG' if self.editor_port_mode == 'ADD' else 'ADD'; self.update_selection_panel()
                        elif event.ui_element == self.ui_elements['selection'].get('delete_port_button'): self.editor_port_mode = 'DRAG' if self.editor_port_mode == 'DELETE' else 'DELETE'; self.update_selection_panel()
            if event.type == pygame_gui.UI_HORIZONTAL_SLIDER_MOVED: self.handle_slider_move(event.ui_element)
            if event.type == pygame_gui.UI_TEXT_ENTRY_FINISHED:
                if event.ui_element == self.editor_title_entry: self.shorts_title_text = event.text
                else: self.handle_text_entry(event.ui_element)
            if event.type == pygame_gui.UI_DROP_DOWN_MENU_CHANGED:
                ...
                for team_id in range(len(TEAMS)):
                    if event.ui_element == self.ui_elements['alliance'].get(f'team_{team_id}_dropdown'):
                        self.alliance_map[team_id] = int(event.text.split(' ')[1]) - 1; self._update_simulation_alliances(); is_alliance_dropdown = True; break
                if not is_alliance_dropdown and self.selected_object:
                    if event.ui_element == self.ui_elements['selection'].get('team_dropdown'): self.selected_object.update_attributes(team_name=event.text)
                    if event.ui_element == self.ui_elements['selection'].get('shape_dropdown'): self.selected_object.update_attributes(shape_name=event.text)
                    self.update_selection_panel()
            if not ui_consumed_event:
                self.viewport.handle_event(event)
                if event.type in [pygame.MOUSEBUTTONDOWN, pygame.MOUSEBUTTONUP, pygame.MOUSEMOTION]:
                    if self.current_mode == 'EDITOR': self.handle_editor_mouse_events(event)
                    else:
                        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                            grid_pos = self.viewport.get_grid_pos(event.pos); self.select_object_at(grid_pos[0], grid_pos[1]) if grid_pos else self.select_object_at(None, None)
        return time_delta

    def run(self):
        while self.is_running:
            time_delta = self.handle_events(); self.ui_manager.update(time_delta); self.vfx_manager.update_effects()
            if not self.is_paused:
                for _ in range(self.sim_speed): self.simulation.step(self.frame_count); self.frame_count += 1
            if self.current_mode == 'EDITOR':
                self.simulation.render_grid.fill(EMPTY); self.simulation.draw_bases_to_grid()
            self.update_layout(); self.update_stats_panel()
    ...
    def select_object_at(self, world_y, world_x):
        ...
        if self.selected_object != clicked_object:
            self.selected_object = clicked_object; self.is_editing_spawns = False; self.update_selection_panel()
            
    def reset_simulation(self):
        for base in self.simulation.bases: base.recalculate_geometry(final_calculation=True, regenerate_ports=False)
        self.simulation.reset_dynamic_state(); self.vfx_manager.particles.clear(); self.frame_count = 0
        if self.current_mode == 'SIMULATION': self.is_paused = False; self.play_pause_button.set_text('PAUSE')
    ...
    def update_layout(self):
        ...
        self.right_panel.set_dimensions((DASHBOARD_RIGHT_PANEL_WIDTH, screen_h)); self.right_panel.set_position((screen_w - DASHBOARD_RIGHT_PANEL_WIDTH, 0))
        bottom_panel_width = screen_w - DASHBOARD_RIGHT_PANEL_WIDTH
        self.bottom_panel.set_dimensions((bottom_panel_width, DASHBOARD_BOTTOM_PANEL_HEIGHT)); self.bottom_panel.set_position((0, screen_h - DASHBOARD_BOTTOM_PANEL_HEIGHT))
    ...
    def toggle_pheromone_display(self):
        self.show_pheromones = not self.show_pheromones; self.toggle_pheromones_button.set_text(f"Pheromones: {'ON' if self.show_pheromones else 'OFF'}")

    def toggle_pause(self):
        if self.current_mode == 'EDITOR': return
        self.is_paused = not self.is_paused; self.play_pause_button.set_text('PLAY' if self.is_paused else 'PAUSE')

    def cycle_speed(self):
        current_index = self.speed_options.index(self.sim_speed); next_index = (current_index + 1) % len(self.speed_options)
        self.sim_speed = self.speed_options[next_index]; self.speed_button.set_text(f'{self.sim_speed}x')

    def handle_slider_move(self, slider):
        slider_id = slider.object_ids[-1].replace('#', '').replace('_slider', ''); current_value = slider.get_current_value()
        if slider_id in self.ui_elements['global']:
            ...
            if hasattr(self.config, slider_id): setattr(self.config, slider_id, current_value); self.simulation._compile_team_params()
        elif slider_id in self.ui_elements['presentation']:
            param = self.ui_elements['presentation'][slider_id]
            if param['is_int']: param['label'].set_text(str(int(current_value)))
            else: param['label'].set_text(f"{current_value:.2f}")
            if hasattr(self.presentation_params, slider_id): setattr(self.presentation_params, slider_id, current_value)
        elif slider_id == 'size' and self.selected_object:
            param = self.ui_elements['selection']['size']; size_val = int(current_value)
            new_scale = param['map'].get(size_val, 1.0); param['label'].set_text(param['text_map'].get(size_val, "Custom"))
    ...
    def handle_text_entry(self, entry_line):
        ...
        base = self.selected_object; entry_id = entry_line.object_ids[-1].replace('#', '').replace('_entry', '')
        try:
            if entry_id in ['spawn_rate', 'units_per_spawn']:
                new_value = int(float(entry_line.get_text()))
                if base.team_id not in self.simulation.team_params_overrides: self.simulation.team_params_overrides[base.team_id] = {}
                self.simulation.team_params_overrides[base.team_id][entry_id] = new_value; self.simulation._compile_team_params()
            elif entry_id in ['core_thickness', 'armor_thickness']:
                new_value = max(1, int(float(entry_line.get_text())))
                setattr(base, entry_id, new_value); base.recalculate_geometry(final_calculation=False, regenerate_ports=False)
        except (ValueError, TypeError): self.update_selection_panel()
    
    def update_stats_panel(self):
        for element in self.stats_container.get_container().elements[:]: element.kill()
        active_teams = sorted(list(set(base.team_name for base in self.simulation.bases)))
        if not active_teams: return
        num_teams = len(active_teams); container_width = self.stats_container.get_container().get_rect().width
        block_width = (container_width - (10 * (num_teams + 1))) / num_teams if num_teams > 0 else container_width - 20
        x_offset = 10
        for team_name in active_teams:
            agent_count = self.simulation.get_team_agent_count(team_name); base_health = self.simulation.get_team_base_health(team_name)
            team_color_hex = '#ffffff'
            for team_data in TEAMS:
                if team_data['name'].lower() == team_name.lower(): team_color_hex = '#%02x%02x%02x' % team_data['color']; break
    ...
    def save_layout_to_file(self):
        ...
        try:
            with open('base_layouts.json', 'w') as f: json.dump(layout_data, f, indent=4)
            print("SUCCESS: Current base layout saved as default.")
        except Exception as e: print(f"ERROR: Could not save layout to 'base_layouts.json'. {e}")
    ...
    def save_port_layout_as_default(self):
        ...
        except Exception as e: print(f"ERROR: Could not save shape template. {e}")
    ...
    def handle_editor_mouse_events(self, event):
            ...
            if self.is_editing_spawns and self.selected_object:
                ...
                if self.editor_port_mode == 'DELETE':
                    ...
                    if (world_y, world_x) not in base.all_base_pixels:
                        rel_y, rel_x = world_y - base.pivot[0], world_x - base.pivot[1]; base._relative_exit_ports.append((rel_y, rel_x))
                    return
                for i, (port_y, port_x) in enumerate(base.exit_ports):
        ...
        if event.type == pygame.MOUSEMOTION and self.dragged_object:
            if self.drag_type == 'base':
                ...
                snapped_y = round(new_y / EDITOR_GRID_SNAP_SIZE) * EDITOR_GRID_SNAP_SIZE; snapped_x = round(new_x / EDITOR_GRID_SNAP_SIZE) * EDITOR_GRID_SNAP_SIZE
                self.dragged_object.pivot = (int(snapped_y), int(snapped_x)); self.dragged_object.recalculate_preview()
            elif self.drag_type == 'spawn_port':
                base = self.dragged_object; new_port_y, new_port_x = world_y - self.drag_offset[0], world_x - self.drag_offset[1]
